...
public class OAuthInterceptor implements Interceptor {
    ...
    private OAuthInterceptor(String consumerKey, String consumerSecret,String token,String secret) {
        this.devToken = token;
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if(original.url().encodedQuery()!=null) {
            generatedBaseString = original.url().encodedQuery() + "&oauth_consumer_key=" + consumerKey + "&oauth_token=" + devToken + "&oauth_nonce=" + nonce + "&oauth_signature_method=HMAC-SHA1&oauth_timestamp=" + timestamp + "&oauth_version=1.0";
        }
        else
        {
            generatedBaseString = "oauth_consumer_key=" + consumerKey + "&oauth_token=" +  devToken +"&oauth_nonce=" + nonce + "&oauth_signature_method=HMAC-SHA1&oauth_timestamp=" + timestamp + "&oauth_version=1.0";
        ...
        String signature = new HMACSha1SignatureService().getSignature(baseString, consumerSecret, devSecret);
        ...
        HttpUrl url = originalHttpUrl.newBuilder()
                ...
                .addQueryParameter(OAUTH_TOKEN , devToken)
    ...
    public static final class Builder {
        ...
        public Builder tokenFunction(String token){
            if(token == null)throw new NullPointerException("token = null");
            this.devToken = token;
            return this;
        }

        public Builder secretFunction(String secret){
            if(secret == null)throw new NullPointerException("token = null");
            this.devSecret = secret;
            return this;
        }
        ...
        public OAuthInterceptor build() {
            ...
            if(devToken == null)throw new IllegalStateException("token not set");
            if(devSecret == null)throw new IllegalStateException("secret not set");

            return new OAuthInterceptor(consumerKey, consumerSecret,devToken,devSecret);
        }
    }
